<template>
  <div v-click-outside="hideAvatarDropdown" class="mt-1 relative md-min:flex md-min:items-center md-min:justify-start w-full rounded-full">
    <div @click="open = !open" class="dropdown-avatar cursor-pointer select-none relative z-10 block h-10 w-10 rounded-full overflow-hidden border-4 border-gray-300 focus:outline-none">
    ...
    <div v-show="open" class="account-dropdown w-full flex-1 absolute right-0 mt-4 py-2 w-64 bg-white rounded-none shadow-dropdown block">
      <div class="block px-4 py-2 hover:bg-gray-200 cursor-pointer mx-2 rounded-none flex justify-start items-center transition ease-in-out duration-300">
        <button class="relative z-10 block h-12 w-12 rounded-full overflow-hidden border-4 border-gray-300 focus:outline-none">
        ...
        <div class="flex flex-col justify-start items-start pl-2">
          <span class="select-none font-bold text-sm text-gray-700">Melinda Gates</span>
          <span class="select-none text-gray-600 text-sm">Graphic Designer</span>
        </div>
      </div>

      <hr class="mx-6 my-2">

      <div class="flex items-center justify-between w-auto px-4 py-2 mx-2 user-presence">
        <button @click="status = 'online'" class="w-full bg-white px-2 border-1 text-gray-700 border-gray-500 focus:outline-none" :class="status == 'online' ? 'active' : ''">Online</button>
        <button @click="status = 'offline'" class="w-full bg-white px-2 border-1 text-gray-700 border-gray-500 focus:outline-none" :class="status == 'offline' ? 'active' : ''">Offline</button>
      </div>
      
      <hr class="mx-6 my-2">

      <div class="flex items-center justify-start w-auto px-4 py-2 hover:bg-gray-200 mx-2 cursor-pointer rounded-none transition ease-in-out duration-300">
        ...
        <span class="pl-3 text-gray-600 font-semibold">Settings</span>
      </div>
      ...
      <div class="flex items-center justify-start w-auto px-4 py-2 hover:bg-gray-200 mx-2 cursor-pointer rounded-none transition ease-in-out duration-300">
        ...
        <span class="pl-3 text-gray-600 font-semibold">Support</span>
      </div>
      ...
      <div class="flex items-center justify-start w-auto px-4 py-2 hover:bg-gray-200 mx-2 cursor-pointer rounded-none transition ease-in-out duration-300">
        ...
        <span class="pl-3 text-gray-600 font-semibold">Community</span>
      </div>
      ...
      <div class="flex items-center justify-start w-auto px-4 py-2 hover:bg-gray-200 mx-2 cursor-pointer rounded-none transition ease-in-out duration-300">
        ...
        <span class="pl-3 text-gray-600 font-semibold">Logout</span>
      </div>
    </div> 
  </div>
</template>
