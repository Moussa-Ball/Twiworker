  ...
  <div v-click-outside="hideAvatarDropdown" class="mt-1 relative md-min:flex md-min:items-center md-min:justify-start w-full rounded-full">
    ...
    <div v-show="open" class="account-dropdown w-full flex-1 absolute right-0 mt-4 py-2 w-64 bg-white rounded-none shadow-dropdown block">
      ...
      <div class="flex items-center justify-between w-auto px-4 py-2 mx-2 user-presence">
        <button @click="status = 'online'" class="w-full bg-white px-2 border-1 text-sm font-normal text-gray-800 border-gray-500 focus:outline-none" :class="status == 'online' ? 'active' : ''">Online</button>
        <button @click="status = 'offline'" class="w-full bg-white px-2 border-1 text-sm font-normal text-gray-800 border-gray-500 focus:outline-none" :class="status == 'offline' ? 'active' : ''">Offline</button>
      </div>
