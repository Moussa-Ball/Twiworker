<template>
  <div
    v-click-outside="hideAvatarDropdown"
    class="lg:z-20 mt-1 relative pb-2 h-auto w-full rounded-full"
  >
    <div
      class="hidden dropdown-avatar cursor-pointer select-none relative z-10 lg:block h-10 w-10 rounded-full overflow-hidden border-4 border-gray-300 focus:outline-none"
      @click="open = !open"
    >
      <img
        class="select-none h-full w-full object-cover rounded-full"
    ...
    >
      ...
      <div class="flex items-center justify-between w-auto lg:mx-6 user-presence shadow-dropdown">
        <button
          class="w-full h-full px-4 py-2 bg-white border-1 text-sm font-normal border-gray-500 focus:outline-none"
          :class="status == 'online' ? 'bg-primary text-white' : 'text-gray-800'"
          @click="status = 'online'"
        >
          Online
        </button>
        <button
          class="w-full h-full px-4 py-2 bg-white px-2 border-1 text-sm font-normal border-gray-500 focus:outline-none"
          :class="status == 'offline' ? 'bg-primary text-white' : 'text-gray-800'"
      ...
      <div class="flex items-center justify-start w-auto px-1 lg:px-4 py-2 hover:bg-gray-200 lg:mx-2 cursor-pointer rounded-none transition ease-in-out duration-300">
        ...
        <span class="pl-3  text-sm text-gray-600 font-medium">Support</span>
      </div>
      ...
      <div class="flex items-center justify-start w-auto px-1 lg:px-4 py-2 hover:bg-gray-200 lg:mx-2 cursor-pointer rounded-none transition ease-in-out duration-300">
        ...
        <span class="pl-3  text-sm text-gray-600 font-medium">Community</span>
      </div>

      <hr class="lg:mx-6 my-2">
      
      <div
        class="flex items-center justify-start w-auto px-1 lg:px-4 py-2 hover:bg-gray-200 lg:mx-2 cursor-pointer rounded-none transition ease-in-out duration-300"
      ...
      >
        ...
        <span class="pl-3  text-sm text-gray-600 font-medium">Logout</span>
      </div>
      <form
        class="logout-form hidden"
        action="/logout"
        method="POST"
      >
        <input
